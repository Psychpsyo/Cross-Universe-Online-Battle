...
export function getAutoResponse(requests) {
	...
	if (requests.length == 1) {
		...
		switch (request.type) {
			...
			case "chooseAbilityOrder": {
				if (shouldStaticAbilitiesBeAutoOrdered(request.abilities, request.sourceCards)) {
					const response = [];
	...
	// on only real option (no retiring partners, no waiting for casting spells from a selection of 0)
...
function shouldStaticAbilitiesBeAutoOrdered(abilities, sourceCards) {
	const affectedProperties = [];
	for (let i = 0; i < abilities.length; i++) {
		const player = sourceCards[i].currentOwner();
		const modifier = abilities[i].modifier.evalFull(new ScriptContext(sourceCards[i], player, abilities[i]))[0].get(player);
		for (const modification of modifier.modifications) {
			...
			if (modification instanceof modifiers.ActionReplaceModification) {
				if (abilities[i].mandatory) return false;
